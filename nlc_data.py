...
def get_nlc_train_set(directory):
    ...
    if not (gfile.Exists(train_path + ".x.txt") and gfile.Exists(train_path + ".y.txt")):
        print('Please provide `train.x.txt` & `train.y.txt` files with --data_dir option.')
...
def get_nlc_dev_set(directory):
    ...
    if not (gfile.Exists(dev_path + ".y.txt") and gfile.Exists(dev_path + ".x.txt")):
        print('Please provide `valid.x.txt` & `valid.y.txt` files with --data_dir option.')
...
def initialize_vocabulary(vocabulary_path, bpe=False):
    if gfile.Exists(vocabulary_path):
        ...
        if bpe:
            vocab = dict([(''.join(x.split(' ')), y)
                          for (y, x) in enumerate(rev_vocab)])
        else:
...
def prepare_nlc_data(data_dir, max_vocabulary_size, tokenizer=char_tokenizer, other_dev_path=None):
    ...
    data_to_token_ids(train_path + ".y.txt",
                      y_train_ids_path, vocab_path, tokenizer)
    data_to_token_ids(train_path + ".x.txt",
                      x_train_ids_path, vocab_path, tokenizer)

    # Create token ids for the development data.
    y_dev_ids_path = dev_path + ".ids.y"
    x_dev_ids_path = dev_path + ".ids.x"
    data_to_token_ids(dev_path + ".y.txt", y_dev_ids_path,
                      vocab_path, tokenizer)
    data_to_token_ids(dev_path + ".x.txt", x_dev_ids_path,
                      vocab_path, tokenizer)
