    ...
    def __init__(self, num_units, encoder_output, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):
            with tf.variable_scope("Attn1"):
                ...
                phi_hs2d = tf.tanh(core_rnn_cell_impl._linear(hs2d, num_units, True, 1.0))
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):
            with tf.variable_scope("Attn2"):
                gamma_h = tf.tanh(core_rnn_cell_impl._linear(gru_out, self._num_units, True, 1.0))
            ...
            with tf.variable_scope("AttnConcat"):
                out = tf.nn.relu(core_rnn_cell_impl._linear([context, gru_out], self._num_units, True, 1.0))
...
class NLCModel(object):
    ...
    def setup_loss(self):
        with tf.variable_scope("Logistic"):
            ...
            logits2d = core_rnn_cell_impl._linear(do2d, self.vocab_size, True, 1.0)
    ...
    def setup_beam(self):
        ...
        def beam_step(time, beam_probs, beam_seqs, cand_probs, cand_seqs, *states):
            ...
            total_probs_noEOS = tf.concat([tf.slice(total_probs, [0, 0], [batch_size, nlc_data.EOS_ID]),
                                           ...
                                           tf.slice(total_probs, [0, nlc_data.EOS_ID + 1], [batch_size, self.vocab_size - nlc_data.EOS_ID - 1])], 1)
