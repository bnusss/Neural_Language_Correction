...
tf.app.flags.DEFINE_boolean(
    "sweep", False, "sweep all alpha rates with dev turned on")
tf.app.flags.DEFINE_boolean(
    "score", False, "generate csv with language model scores on target and generated.")
...
def network_score(model, sess, encoder_output, target_tokens):
    ...
    for (feed, pick) in zip(list(target_tokens)[:-1], list(target_tokens)[1:]):
        scores, _, states = model.decode(
            sess, encoder_output, np.array([feed]), None, states)
...
def setup_batch_decode(sess):
    ...
    vocab, reverse_vocab = nlc_data.initialize_vocabulary(
        vocab_path, bpe=(FLAGS.tokenizer.lower() == "bpe"))
...
def batch_decode(model, sess, x_dev, y_dev, alpha):
    ...
    for source_tokens, source_mask, target_tokens, target_mask in pair_iter(x_dev, y_dev, 1,
        ...
        tgt_nw_score = network_score(
            model, sess, encoder_output, target_tokens)
